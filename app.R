...
ui <- dashboardPage( 
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem("3BP",
              fluidRow(
                box( title = "Daily AQI Data Line Chart", solidHeader = TRUE, status = "primary", width = 12, plotOutput("lineChartAQI"))
              )
      ),
      tabItem("about",
...
server <- function(input, output, session) 
{
  ...
  output$lineChartAQI <- renderPlot({
    ggplot(AQIstatus(), aes(x=Days, y=AQI)) # This needs to be the day on the x axis and AQI on the y
  })
